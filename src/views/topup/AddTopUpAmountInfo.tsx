...
const AddTopUpAmountInfo = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (Object.keys(error).length === 0) {
      ...
      const data = await productService.addAmount(amounts, 'topUps');
      if (data) {
        getAmount();
      }
  ...
  const getAmount = async () => {
    const data = await productService.getAmount('topUps');
    if (data) {
      ...
      setAmount(data1);
      setTag(data1);
      // setInputCount(data1.length ? data1.length : 2);
      setFlag(false);
      setLoading(false);
    }
  };
...
export default AddTopUpAmountInfo;
